...
const POSITIONS: Record<string, { x: number; y: number; scale: number }> = {
  home:   { x: W * 0.38, y: H * 0.48, scale: 2.8 },
  walk:   { x: W * 0.62, y: H * 0.28, scale: 2.4 },
  sleep:  { x: W * 0.10, y: H * 0.32, scale: 2.2 },
  screen: { x: W * 0.64, y: H * 0.67, scale: 2.1 },
  learn:  { x: W * 0.09, y: H * 0.70, scale: 2.1 },
  quest:  { x: W * 0.36, y: H * 0.16, scale: 2.0 },
};
    ...
    <WaterBg>
      <SafeAreaView style={{ flex: 1 }}>
        ...
        <View style={{ flex: 1, position: 'relative' }}>
          ...
          {/* Home island */}
          <View style={{ position: 'absolute', left: home.x - home.scale * 16, top: home.y - home.scale * 30, alignItems: 'center' }}>
            {!swimming && <Blob state="healthy" scale={2.4} />}
            <Island type="walk" state="thriving" scale={home.scale} />

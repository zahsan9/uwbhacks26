  ...
  return (
    <WorldBg>
      <div style={{ position: 'absolute', top: 56, left: 24, right: 24, zIndex: 6, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <div className="vq-eyebrow">Your world</div>
          <div className="vq-h2" style={{ marginTop: 4 }}>{layout === 'scattered' ? 'Archipelago' : 'Linear'}</div>
        </div>
        <button style={{ background: 'transparent', border: '2px solid var(--vq-border-strong)', borderRadius: 4, width: 36, height: 36, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
          <IconFriends size={16} color="var(--vq-ink)" />
        </button>
      </div>
      {avatarPos.island !== 'home' && (
        <svg style={{ position: 'absolute', inset: 0, zIndex: 2, pointerEvents: 'none' }} width="100%" height="100%">
          {[0.2, 0.4, 0.6, 0.8].map((t, i) => (
            <circle key={i}
              cx={from.left + 44 + (to.left - from.left) * t}
              cy={from.top + 36 + (to.top - from.top) * t}
              r="2.5" fill="var(--vq-ink)" opacity={0.4}>
              <animate attributeName="opacity" from="0" to="0.6" dur="1.5s" begin={`${i * 0.2}s`} repeatCount="indefinite" />
            </circle>
          ))}
        </svg>
      )}
        ...
        return (
          <div key={key} style={{ position: 'absolute', top: pos.top, left: pos.left, zIndex: 3 }}>
            ...
            <div style={{
              ...
              color: locked ? 'var(--vq-ink-dim)' : 'var(--vq-ink)',
      ...
      <div style={{ position: 'absolute', top: positions.home.top, left: positions.home.left, zIndex: 4 }}>
        ...
        <div style={{
          position: 'absolute', bottom: -18, left: '50%', transform: 'translateX(-50%)',
          fontFamily: 'Fredoka', fontWeight: 500, fontSize: 11, color: 'var(--vq-ink)',
      ...
      <div style={{ position: 'absolute', bottom: 40, left: 24, right: 24, zIndex: 6, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div className="vq-small">Tap an island to visit</div>
        <div className="vq-small">Health 78</div>
      </div>
    </WorldBg>
  );
}
...
Object.assign(window, {
  WorldBg, OnboardingSignup, OnboardingHealth, OnboardingAvatar, OnboardingHabits,
